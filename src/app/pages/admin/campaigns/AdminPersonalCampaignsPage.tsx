    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-semibold text-gray-900 text-xl">Персональные кампании</h1>
            <p className="text-sm text-gray-500 mt-0.5">{rows.length} кампаний</p>
          </div>
        </div>
        <Link
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4 text-xs text-blue-900 leading-relaxed">
        <b>Персональная кампания</b> — это правило выдачи индивидуального оффера конкретному пользователю. Система сама
        выбирает кампанию по контексту (что покупал, чего не хватает в рутине). Сами скидки задаются внутри в виде{' '}
        <b>офферов</b> с собственным таргетингом и cooldown.
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-50">
            {loading ? (
              ...
              <tr>
                <td className="px-5 py-6 text-sm text-gray-500" colSpan={8}>
                  Кампаний нет. Создайте первую — она появится тут.
                </td>
              </tr>
            ) : (
              rows.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-4 text-xs text-gray-700">{c.priority}</td>
                  <td className="px-4 py-4">
                    {c.offersCount > 0 ? (
                  ...
                  <td className="px-4 py-4">
                    <Link
                      to={`/admin/campaigns/personal/${c.id}`}
