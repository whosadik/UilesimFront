...
const promoCardCopy = {
  ru: {
    ...
    description: 'Ответьте на несколько вопросов и получите персональное предложение.',
    button: 'Открыть',
  },
  kk: {
    badge: 'Сізге арнайы',
    title: 'Жеке оффер',
    description: 'Бірнеше сұраққа жауап беріп, жеке ұсыныс алыңыз.',
    button: 'Ашу',
  },
  en: {
    badge: 'Personal for you',
    title: 'Personal offer',
    description: 'Answer a few questions and get a personal offer.',
    button: 'Open',
  },
} as const;
...
export function PromoCard({
  ...
  const copy = promoCardCopy[language];
  const resolvedBadge = badgeText ?? copy.badge;
  const resolvedTitle = (typeof title === 'string' && title.trim()) || (typeof name === 'string' && name.trim()) || copy.title;
  const resolvedDescription = (typeof description === 'string' && description.trim()) || (typeof subtitle === 'string' && subtitle.trim()) || copy.description;
  const resolvedButtonText = (typeof buttonText === 'string' && buttonText.trim()) || (typeof ctaLabel === 'string' && ctaLabel.trim()) || copy.button;
    ...
    <div className="relative rounded-3xl p-8 bg-white/90 backdrop-blur-lg border border-[#EAE6EF] shadow-lg overflow-hidden">
      ...
      <div className="relative z-10">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#FFE1F2] border border-[#FF4DB8] mb-4">
          ...
          <span className="text-xs font-medium text-[#FF4DB8]">{resolvedBadge}</span>
        </div>
        ...
        <Button variant="primary" onClick={onAction} disabled={buttonDisabled}>{resolvedButtonText}</Button>
        {hint && (
